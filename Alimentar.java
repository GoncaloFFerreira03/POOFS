public class Alimentar extends Produto {
    ...
    private void setTipoDeTaxa() {
        ArrayList<String> certificados = {"ISO22000","FSSC22000","HACCP","GMP"};
        ArrayList<String> categorias = {"Congelado", "Enlatado", "Vinho"};
        int listSize= this.certOuCat.length;
        if(listSize>0){
            for(String categoria: categorias){
                if(this.certOuCat[0].equals(categoria) {//vamos assumir que se for categoria, nao precisamos de ver mais
                    this.tipoDeTaxa = "intermédia";
                    break;
                }
            }
            for(String categoria: categorias){
                if(this.certOuCat[0].equals(categoria) {//como no mázimo so há 4, e nesta condição sabemos que existe pelo menos uma, nao precisamos de ver mais
    ...
    private int calcularTaxaExtra(){
        ...
        if(this.tipoDeTaxa.equals("reduzida")){
            ArrayList<String> certificacoesValidas = new ArrayList<>(List.of("ISO22000", "FSSC22000", "HACCP", "GMP"));
            if (certOuCat.containsAll(certificacoesValidas)) {
                taxaExtra -= 1; // Redução de 1%
            }
        }
        else if(this.tipoDeTaxa.equals("intermédia")){
            if (certOuCat[0].equals("Vinho")) {
    ...
    public int calcularTaxa(local){
        ...
        if(local.equals("Portugal Continental"))   {
            if(this.tipoDeTaxa.equals("reduzida")){
                ...
                taxaTotal = 13;
            }else if(this.tipoDeTaxa.equals("reduzida")) {
                taxaTotal = 23;
            }
        }
